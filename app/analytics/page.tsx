...
async function fetchAnalytics(range: DateRangeKey, from?: string, to?: string): Promise<AnalyticsData> {
  ...
  const { data: visits } = await supabase
    .from('visits')
    .select('created_at, outcome, city, rep_name, lead_id, business_type')
  ...
  // Per-rep performance
  const repMap: Record<string, { doors: number; leads: number; lead_ids: (string | null)[] }> = {}
  for (const v of rows) {
    const r = v.rep_name || 'Unknown'
    if (!repMap[r]) repMap[r] = { doors: 0, leads: 0, lead_ids: [] }
    repMap[r].doors++
    if (v.outcome === 'lead_captured') {
      repMap[r].leads++
      repMap[r].lead_ids.push(v.lead_id)
    }
  }

  const reps = Object.entries(repMap)
    .map(([rep, stats]) => {
      const hot = stats.lead_ids.filter(id => id && hotLeadIdSet.has(id)).length
      return {
        rep,
        doors: stats.doors,
        leads: stats.leads,
  ...
  return { kpi: { total_doors, leads_collected, hot_leads, hot_lead_pct }, cities, matrix, reps, businessTypes }
}
...
export default async function AnalyticsPage({
  ...
  const { kpi, cities, matrix, reps, businessTypes } = data
  ...
  return (
    <main className="min-h-screen bg-gray-100 py-6 px-4">
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        {reps.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 p-4">
            <h2 className="text-sm font-semibold text-gray-700 mb-3">By Rep</h2>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left">
                    <th className="text-gray-500 font-medium pb-2 pr-3">Rep</th>
                    ...
                    <th className="text-gray-500 font-medium pb-2 px-2 text-center">Conv %</th>
                    <th className="text-gray-500 font-medium pb-2 pl-2 text-center">🔥 Hot</th>
                  </tr>
                </thead>
                <tbody>
                  {reps.map(row => (
                    <tr key={row.rep} className="border-t border-gray-100">
                      <td className="py-2 pr-3 text-gray-700 font-medium whitespace-nowrap">{row.rep}</td>
                      ...
                      <td className="py-2 px-2 text-center">
                        <span className={`font-semibold ${row.conversion_pct >= 20 ? 'text-green-600' : row.conversion_pct >= 10 ? 'text-yellow-600' : 'text-gray-500'}`}>
                          {row.conversion_pct}%
                        </span>
                      </td>
                      <td className="py-2 pl-2 text-center font-semibold text-orange-600">{row.hot_leads > 0 ? row.hot_leads : <span className="text-gray-300">—</span>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
